...
gun_crime <- read.csv("Data/US gun crime - SUMMARY 2011.csv",
                      stringsAsFactors = FALSE
)
gun_ownership <- read.csv("Data/gun_ownership.csv",
                          stringsAsFactors = FALSE
)
gun_ownership$percown <- as.numeric(gsub("%", "", gun_ownership$percown))
background_checks <- read.csv("Data/nics-firearm-background-checks.csv",
                              stringsAsFactors = FALSE
)
state_deaths <- read.csv("Data/firearm2016.csv",
                         stringsAsFactors = FALSE
)
...
shootings <- mass_shootings %>%
  group_by(state) %>%
  summarise(
    fatalities = sum(fatalities), injured = sum(injured),
...
crime <- select(
  joined_data, Violent.Crime...per.100.000..2013, state,
  Murder.and.nonnegligent.manslaughter..per.100K..2013,
  percown, industry_score
)
...
my_server <- function(input, output) {
  ...
  output$scatter_plot <- renderPlotly({
    ...
    p <- plot_ly(
      ...
      add_markers(
        y = y,
        text = paste(
          joined_data$state, "<br>", "Happiness Rank:",
      ...
      add_lines(
        x = joined_data$happiness_score,
        y = fitted(lm(y ~ joined_data$happiness_score)),
  ...
  output$cor_message <- renderText({
    if (input$yaxis == "percown") {
      ...
      score, the lower the percentage of gun ownership."
    } else if (input$yaxis == "rate") {
      message <- "This negative correlation shows that the higher the happiness
      score, the lower the rate of deaths per 100k people."
    } else if (input$yaxis == "lawtotal") {
      message <- "This positive correlation shows that there is an association
      between higher happiness score and a higher count of gun control laws."
    }
    return(message)
    })
  ...
  output$legis_cor_message <- renderText({
    if (input$legislation == 1) {
      ...
      gun control negatively impacts the state's gun industry."
    } else if (input$legislation == 2) {
      message <- "This moderate negative correlation shows that state
      gun control somewhat reduces state gun ownership."
    } else if (input$legislation == 3) {
      message <- "This slight negative correlation shows that state
      gun control has a minimal effect on reducing the rate of violent crime"
    } else {
      message <- "This slight negative correlation shows that state gun control
      has a small impact on reducing the rate of murder and manslaughter"
    }
    return(message)
    })
